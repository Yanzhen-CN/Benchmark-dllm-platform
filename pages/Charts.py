...
from platform_core.profiling import configured_profiling_models
...
def command_for(preset: str) -> list[str]:
    ...
    if preset == "Sudoku trace comparison":
        left_model = st.session_state.get("compare_left_model", "llada2_1")
        left_variant = st.session_state.get("compare_left_variant", "qmode")
        right_model = st.session_state.get("compare_right_model", "diffusiongemma")
        right_variant = st.session_state.get("compare_right_variant", "official")
        dataset = st.session_state.get("compare_trace_dataset", "")
        sample_id = st.session_state.get("compare_trace_sample", "")
        return [
            *command,
            "--preset",
            "sudoku-trace-batch",
            ...
            *dataset_arguments(dataset),
            "--compare-run",
            f"{left_model}/{left_variant}",
            "--compare-run",
            f"{right_model}/{right_variant}",
            "--sample-ids",
            sample_id,
            "--no-report",
        ]
...
def trashed_media_files(
    ...
    for entry in entries:
        payload = OUTPUT / ".trash" / entry["id"] / entry.get("payload_name", "payload")
        ...
        for path in candidates:
            ...
            if original_path.suffix.lower() not in {".png", ".jpg", ".jpeg", ".gif"}:
                continue
...
def render_media(path: Path, caption: str | None = None) -> None:
    zoomable_image(path, caption=caption or path.name)


@st.dialog("Delete visualization")
def confirm_delete_asset(path: Path) -> None:
    ...
    with permanent_column:
        if st.button(
            ...
            type="primary",
            use_container_width=True,
            key="permanently_delete_asset",
        ):
            permanently_delete_output_paths(OUTPUT, [path])
            st.rerun()
...
st.title(tr("可视化图库", "Charts"))

chart_sections = {
    "generate": tr("生成", "Generate"),
    "library": tr("图库", "Library"),
    "trash": tr("回收站", "Recycle Bin"),
}
chart_section = st.segmented_control(
    tr("页面", "Section"),
    list(chart_sections),
    default="generate",
    format_func=chart_sections.get,
    label_visibility="collapsed",
    key="chart_section",
)

if chart_section == "generate":
    descriptions = {
    ...
    with st.container(border=True):
        ...
        if preset == "Profiling comparison":
            available = sorted(
                set(dirs(OUTPUT / "model_profiling"))
                | set(configured_profiling_models(OUTPUT.parent))
            )
        ...
        elif preset == "Sudoku trace comparison":
            model_options = [
                name
                for name in dirs(OUTPUT / "model_output")
                if any(
                    part.startswith("sudoku")
                    for variant in dirs(OUTPUT / "model_output" / name)
                    for part in datasets(name, variant)
                )
            ]
            ...
            with left_column:
                left_model = st.selectbox(
                    "Left model",
                    model_options,
                    index=model_options.index("llada2_1") if "llada2_1" in model_options else 0,
                    key="compare_left_model",
                )
                left_variants = [
                    name
                    for name in dirs(OUTPUT / "model_output" / left_model)
                    if any(part.startswith("sudoku") for part in datasets(left_model, name))
                ]
                left_variant = st.selectbox(
                    "Left variant",
                    left_variants,
                    index=left_variants.index("qmode") if "qmode" in left_variants else 0,
                    key="compare_left_variant",
                )
            with right_column:
                right_model = st.selectbox(
                    "Right model",
                    model_options,
                    index=model_options.index("diffusiongemma") if "diffusiongemma" in model_options else 0,
                    key="compare_right_model",
                )
                right_variants = [
                    name
                    for name in dirs(OUTPUT / "model_output" / right_model)
                    if any(part.startswith("sudoku") for part in datasets(right_model, name))
                ]
                right_variant = st.selectbox(
                    "Right variant",
                    right_variants,
                    index=right_variants.index("official") if "official" in right_variants else 0,
                    key="compare_right_variant",
                )
            common_datasets = sorted(
                set(datasets(left_model, left_variant)).intersection(
                    datasets(right_model, right_variant)
                )
            )
            common_datasets = [name for name in common_datasets if name.startswith("sudoku")]
            dataset_choices = common_datasets or [""]
            ...
            with dataset_column:
                compare_dataset = st.selectbox(
                    ...
                    dataset_choices,
                    index=(
                        dataset_choices.index("sudoku9_1shot_l128")
                        if "sudoku9_1shot_l128" in dataset_choices
                        else 0
                    ),
                    key="compare_trace_dataset",
                    format_func=lambda value: value or "No common dataset",
                )
            if compare_dataset:
                from platform_core.visualization import comparable_sudoku_samples

                common_samples = comparable_sudoku_samples(
                    OUTPUT / "model_output",
                    [
                        f"{left_model}/{left_variant}",
                        f"{right_model}/{right_variant}",
                ...
                common_samples = []
            sample_choices = common_samples or [""]
            with sample_column:
                st.selectbox(
                    "Common sample",
                    sample_choices,
                    index=(
                        sample_choices.index("sudoku4-d1-0021")
                        if "sudoku4-d1-0021" in sample_choices
                        else 0
                    ),
                    key="compare_trace_sample",
                    format_func=lambda value: value or "No common sample",
                )
            if not common_datasets or not common_samples:
                st.warning(
                    "The selected runs do not share a sample where both traces form a complete Sudoku answer."
                )
            ...
            if not model_options:
                st.info("No saved Sudoku traces are available for visualization.")
                st.stop()
        ...
        for path in filtered[:limit]:
            ...
            with st.expander(label, expanded=limit <= 3):
                ...
                if st.button(
                    ...
                    confirm_delete_asset(path)

elif chart_section == "trash":
    ...
    if not trash_entries:
        ...
        kinds = sorted({original.suffix.lower().lstrip(".") for _, _, original in trash_assets})
        type_column, search_column = st.columns([1, 2])
        with type_column:
        ...
        visible_assets = [
            ...
            and (selected_variant == "全部变体" or variant == selected_variant)
            and (selected_dataset == "全部数据集" or dataset == selected_dataset)
            and original.suffix.lower().lstrip(".") in selected_kinds
        ...
        for entry, payload, original in visible_assets[:limit]:
            ...
            with st.expander(
                ...
                render_media(payload, caption=original.name)
