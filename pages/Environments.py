...
st.title(tr("运行环境", "Environments"))

left, right = st.columns(2)
left.metric(tr("平台 Python", "Platform Python"), sys.version.split()[0])
right.metric(tr("模型配置", "Model configurations"), len(discover_models(paths)))

st.subheader(tr("平台环境", "Platform environment"))
st.code(sys.executable)
st.write("这里只安装 Streamlit 和 PyYAML，不混入 torch 或模型特定的 transformers 版本。")

st.subheader(tr("模型启动器", "Model launchers"))
